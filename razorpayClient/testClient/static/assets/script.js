...
find(".loginForm").addEventListener("submit",e=>{
	...
	axios.post("http://localhost:7000/checkoutLogin",{email:email.value,pass:pass.value})
	.then((data)=>{
...
function loggedIn(){
	...
	if(typeof userid==undefined || userid==null){
		...
		$(".login_page").hide()
		$(".order_page").show()
		axios.post("http://localhost:7000/order2",{userid:userid})
		.then(data=>{
...
function submitOrder(e){
  ...
  axios
    .post("http://localhost:7000/checkout", order)
...
//   const { name, orderId, amount } = JSON.parse(sessionStorage.getItem("order"));
  ...
  const checkoutObject = {
    ...
    handler: ({
      ...
      axios
        .post("http://localhost:7000/checkoutVerify", {
          razorpay_order_id,
          razorpay_payment_id,
          razorpay_signature,
          amount: amountInPaise,
          currency: "INR"
			...
			axios.post("http://localhost:7000/orderUpdate",{id:someid})
